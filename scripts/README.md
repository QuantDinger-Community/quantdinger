...
| --- | --- | --- |
| `generate-secret-key.ps1` | Generate a secure `SECRET_KEY` and update `backend/.env` on Windows. | Keep |
| `generate-secret-key.sh` | Generate a secure `SECRET_KEY` and update `backend/.env` on macOS/Linux. | Keep |
| `bump_version.py` | Update the repo-root `VERSION` and `backend/VERSION` fallback files. | Keep |
