...
- **No TWS/Gateway required** â€” pure stateless REST authentication
- **Paper trading built-in** â€” separate paper-api.alpaca.markets endpoint
- **Modern Python SDK** â€” `alpaca-py` is well-maintained, async-friendly
- **Crypto support** â€” same client handles stocks AND crypto (BTC/USD, ETH/USD, etc.)
- **Fractional shares** â€” buy $5 worth of TSLA without owning a full share
...
Get your keys at:
- Paper: https://app.alpaca.markets/paper/dashboard/overview â†’ "Generate New Key"
- Live: https://app.alpaca.markets/brokerage/dashboard/overview â†’ "Generate New Key"
...
Alpaca supports crypto using the same client â€” just pass `market_type="crypto"`:

```python
result = client.place_market_order(
    symbol="BTC/USD",  # or "BTCUSD" â€” auto-normalized
    side="buy",
    quantity=0.01,
    market_type="crypto",
)
...
| Class shares | `BRK/B` â†’ `BRK.B` for REST; do not treat as a crypto pair |

This module normalizes symbols (`symbols.py`): `BTC/USDT` â†’ `BTC/USD` for crypto, `BRK/B` â†’ `BRK.B` for equities.
