...
Interactive Brokers allows **one API connection per `clientId`**. If two programs use the same id, **the newer connection replaces the older** â€” often within a few seconds it looks like a random disconnect, and live orders then fail with â€œFailed to connectâ€.
...
```
backend/app/services/ibkr_trading/    # Entire directory
backend/app/routes/ibkr.py            # Route file
```
